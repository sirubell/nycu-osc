...
void mm_init(const void *fdt) {
    ...
    /* Explicitly reserve space for the User Program so alloc_page() never
     * touches it */
    record_reserved("user-program", USER_LOAD_ADDR,
                    USER_LOAD_ADDR + 0x200000); // Reserve 2MB
    ...
    memset(mem_map, 0, (size_t)fa_bytes);
